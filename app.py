...
def dashboard(state=None):
    ...
    ph = float(ph_data.iloc[-1, -1]) if not ph_data.empty else 7
    tds = float(tds_data.iloc[-1, -1]) if not tds_data.empty else 300
    turbidity = float(turbidity_data.iloc[-1, -1]) if not turbidity_data.empty else 3
    do = float(do_data.iloc[-1, -1]) if not do_data.empty else 7
